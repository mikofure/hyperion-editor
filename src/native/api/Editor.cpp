...
Sci::Position Editor::FormatRange(Hyperion::Message iMessage, Hyperion::uptr_t wParam, Hyperion::sptr_t lParam) {
	if (!lParam)
		return 0;
...
SelectionRange Editor::SelectionMove(Hyperion::Message iMessage, size_t r) {
...
std::unique_ptr<Surface> Editor::CreateDrawingSurface(SurfaceID sid, std::optional<Hyperion::Technology> technologyOpt) const {
	if (!wMain.GetID()) {
		return {};
	}
...
sptr_t Editor::BytesResult(Hyperion::sptr_t lParam, std::string_view sv) noexcept {
...
sptr_t Editor::WndProc(Message iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case Message::PositionFromLine:
		...
		// Replacement of the old Hyperion interpretation of EM_LINELENGTH
