...
#include "../core/EditModel.hpp"
#include "../view/MarginView.hpp"
#include "../view/EditView.hpp"

#include "Editor.hpp"
#include "AutoComplete.hpp"
#include "HyperionBase.hpp"
#include "CallTip.hpp"
...
HyperionBase::~HyperionBase() = default;

void HyperionBase::Finalise() {
	Editor::Finalise();
	popup.Destroy();
}

void HyperionBase::InsertCharacter(std::string_view sv, CharacterSource charSource) {
...
void HyperionBase::Command(int cmdId) {
...
int HyperionBase::KeyCommand(Message iMessage) {
...
void HyperionBase::ListNotify(ListBoxEvent *plbe) {
	switch (plbe->event) {
...
void HyperionBase::MoveImeCarets(Sci::Position offset) noexcept {
...
void HyperionBase::DrawImeIndicator(int indicator, Sci::Position len) {
...
void HyperionBase::AutoCompleteInsert(Sci::Position startPos, Sci::Position removeLen, std::string_view text) {
	UndoGroup ug(pdoc);
...
void HyperionBase::AutoCompleteStart(Sci::Position lenEntered, const char *list) {
...
void HyperionBase::AutoCompleteCancel() {
	if (ac.Active()) {
...
void HyperionBase::AutoCompleteMove(int delta) {
	ac.Move(delta);
}

void HyperionBase::AutoCompleteMoveToCurrentWord() {
...
void HyperionBase::AutoCompleteSelection() {
...
void HyperionBase::AutoCompleteCharacterAdded(char ch) {
...
void HyperionBase::AutoCompleteCharacterDeleted() {
...
void HyperionBase::AutoCompleteNotifyCompleted(char ch, CompletionMethods completionMethod, Sci::Position firstPos, const char *text) {
	NotificationData scn = {};
...
void HyperionBase::AutoCompleteCompleted(char ch, CompletionMethods completionMethod) {
	const int item = ac.GetSelection();
	if (item == -1) {
		AutoCompleteCancel();
		return;
...
int HyperionBase::AutoCompleteGetCurrent() const {
	if (!ac.Active())
		return -1;
	return ac.GetSelection();
}

int HyperionBase::AutoCompleteGetCurrentText(char *buffer) const {
	if (ac.Active()) {
		const int item = ac.GetSelection();
		if (item != -1) {
			const std::string selected = ac.GetValue(item);
...
void HyperionBase::CallTipShow(Point pt, const char *defn) {
	ac.Cancel();
...
void HyperionBase::CallTipClick() {
	NotificationData scn = {};
...
bool HyperionBase::ShouldDisplayPopup(Point ptInWindowCoordinates) const {
	return (displayPopupMenu == PopUp::All ||
		(displayPopupMenu == PopUp::Text && !PointInSelMargin(ptInWindowCoordinates)));
}

void HyperionBase::ContextMenu(Point pt) {
...
void HyperionBase::CancelModes() {
	AutoCompleteCancel();
	ct.CallTipCancel();
	Editor::CancelModes();
}

void HyperionBase::ButtonDownWithModifiers(Point pt, unsigned int curTime, KeyMod modifiers) {
	CancelModes();
	Editor::ButtonDownWithModifiers(pt, curTime, modifiers);
}

void HyperionBase::RightButtonDownWithModifiers(Point pt, unsigned int curTime, KeyMod modifiers) {
	CancelModes();
...
LexState *HyperionBase::DocumentLexState() {
...
void HyperionBase::NotifyStyleToNeeded(Sci::Position endStyleNeeded) {
...
sptr_t HyperionBase::WndProc(Message iMessage, uptr_t wParam, sptr_t lParam) {
	switch (iMessage) {
