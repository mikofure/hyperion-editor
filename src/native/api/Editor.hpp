...
public:
	...
	void Need(WorkItems items_, Sci::Position pos) noexcept {
		if (Hyperion::FlagSet(items_, WorkItems::style) && (upTo < pos))
			upTo = pos;
...
public:
	...
	bool lineCopy;
	int codePage;
	Hyperion::CharacterSet characterSet;
	SelectionText() noexcept : rectangular(false), lineCopy(false), codePage(0), characterSet(Hyperion::CharacterSet::Ansi) {}
	void Clear() noexcept {
		s.clear();
		rectangular = false;
		lineCopy = false;
		codePage = 0;
		characterSet = Hyperion::CharacterSet::Ansi;
	}
	void Copy(const std::string &s_, int codePage_, Hyperion::CharacterSet characterSet_, bool rectangular_, bool lineCopy_) {
		s = s_;
		codePage = codePage_;
...
struct CaretPolicySlop {
	Hyperion::CaretPolicy policy;	// Combination from CaretPolicy::Slop, CaretPolicy::Strict, CaretPolicy::Jumps, CaretPolicy::Even
	int slop;	// Pixels for X, lines for Y
	CaretPolicySlop(Hyperion::CaretPolicy policy_, intptr_t slop_) noexcept :
		policy(policy_), slop(static_cast<int>(slop_)) {}
	CaretPolicySlop(uintptr_t policy_=0, intptr_t slop_=0) noexcept :
		policy(static_cast<Hyperion::CaretPolicy>(policy_)), slop(static_cast<int>(slop_)) {}
};
...
struct VisiblePolicySlop {
	Hyperion::VisiblePolicy policy;	// Combination from VisiblePolicy::Slop, VisiblePolicy::Strict
	int slop;	// Pixels for X, lines for Y
	VisiblePolicySlop(uintptr_t policy_ = 0, intptr_t slop_ = 0) noexcept :
		policy(static_cast<Hyperion::VisiblePolicy>(policy_)), slop(static_cast<int>(slop_)) {}
};
...
protected:	// HyperionBase subclass needs access to much of Editor

	/** On GTK+, Hyperion is a container widget holding two scroll bars
	 * whereas on Windows there is just one window with both scroll bars turned on. */
	Window wMain;	///< The Hyperion parent window
	...
	Hyperion::CursorShape cursorMode;
	...
	bool endAtLastLine;
	Hyperion::CaretSticky caretSticky;
	Hyperion::MarginOption marginOptions;
	bool mouseSelectionRectangularSwitch;
	bool multipleSelection;
	bool additionalSelectionTyping;
	Hyperion::MultiPaste multiPasteMode;

	Hyperion::VirtualSpace virtualSpaceOptions;
	...
	Hyperion::FindOption searchFlags;
	Sci::Line topLine;
	Sci::Position posTopLine;
	Sci::Position lengthForEncode;

	Hyperion::Update needUpdateUI;
	...
	Hyperion::IdleStyling idleStyling;
	bool needIdleStyling;

	Hyperion::ModificationFlags modEventMask;
	bool commandEvents;
	...
	Hyperion::AutomaticFold foldAutomatic;
	...
	struct LineDocSub {
		Hyperion::Line lineDoc = 0;
		Hyperion::Line subLine = 0;
	};
	...
	bool UserVirtualSpace() const noexcept {
		return (FlagSet(virtualSpaceOptions, Hyperion::VirtualSpace::UserAccessible));
	}
	...
	Sci::Position FormatRange(Hyperion::Message iMessage, Hyperion::uptr_t wParam, Hyperion::sptr_t lParam);
	long TextWidth(Hyperion::uptr_t style, const char *text);
	...
	virtual void InsertCharacter(std::string_view sv, Hyperion::CharacterSource charSource);
	...
	virtual void NotifyParent(Hyperion::NotificationData scn) = 0;
	virtual void NotifyStyleToNeeded(Sci::Position endStyleNeeded);
	void NotifyChar(int ch, Hyperion::CharacterSource charSource);
	void NotifySavePoint(bool isSavePoint);
	void NotifyModifyAttempt();
	virtual void NotifyDoubleClick(Point pt, Hyperion::KeyMod modifiers);
	void NotifyHotSpotClicked(Sci::Position position, Hyperion::KeyMod modifiers);
	void NotifyHotSpotDoubleClicked(Sci::Position position, Hyperion::KeyMod modifiers);
	void NotifyHotSpotReleaseClick(Sci::Position position, Hyperion::KeyMod modifiers);
	bool NotifyUpdateUI();
	void NotifyPainted();
	void NotifyIndicatorClick(bool click, Sci::Position position, Hyperion::KeyMod modifiers);
	bool NotifyMarginClick(Point pt, Hyperion::KeyMod modifiers);
	bool NotifyMarginRightClick(Point pt, Hyperion::KeyMod modifiers);
	...
	void NotifyErrorOccurred(Document *doc, void *userData, Hyperion::Status status) override;
	void NotifyGroupCompleted(Document *, void *) noexcept override;
	void NotifyMacroRecord(Hyperion::Message iMessage, Hyperion::uptr_t wParam, Hyperion::sptr_t lParam);

	void ContainerNeedsUpdate(Hyperion::Update flags) noexcept;
	...
	SelectionPosition PositionMove(Hyperion::Message iMessage, SelectionPosition spCaretNow);
	SelectionRange SelectionMove(Hyperion::Message iMessage, size_t r);
	int HorizontalMove(Hyperion::Message iMessage);
	int DelWordOrLine(Hyperion::Message iMessage);
	virtual int KeyCommand(Hyperion::Message iMessage);
	virtual int KeyDefault(Hyperion::Keys /* key */, Hyperion::KeyMod /*modifiers*/);
	int KeyDownWithModifiers(Hyperion::Keys key, Hyperion::KeyMod modifiers, bool *consumed);

	void Indent(bool forwards, bool lineIndent);

	virtual std::unique_ptr<CaseFolder> CaseFolderForEncoding();
	Sci::Position FindText(Hyperion::uptr_t wParam, Hyperion::sptr_t lParam);
	Sci::Position FindTextFull(Hyperion::uptr_t wParam, Hyperion::sptr_t lParam);
	void SearchAnchor() noexcept;
	Sci::Position SearchText(Hyperion::Message iMessage, Hyperion::uptr_t wParam, Hyperion::sptr_t lParam);
	...
	void MouseLeave();
	virtual void ButtonDownWithModifiers(Point pt, unsigned int curTime, Hyperion::KeyMod modifiers);
	virtual void RightButtonDownWithModifiers(Point pt, unsigned int curTime, Hyperion::KeyMod modifiers);
	void ButtonMoveWithModifiers(Point pt, unsigned int curTime, Hyperion::KeyMod modifiers);
	void ButtonUpWithModifiers(Point pt, unsigned int curTime, Hyperion::KeyMod modifiers);
	...
	constexpr bool SynchronousStylingToVisible() const noexcept {
		return (idleStyling == Hyperion::IdleStyling::None) || (idleStyling == Hyperion::IdleStyling::AfterVisible);
	}
	void IdleStyle();
	virtual void IdleWork();
	virtual void QueueIdleWork(WorkItems items, Sci::Position upTo=0);

	virtual int SupportsFeature(Hyperion::Supports feature);
	...
	void SetAnnotationVisible(Hyperion::AnnotationVisible visible);
	void SetEOLAnnotationVisible(Hyperion::EOLAnnotationVisible visible);

	Sci::Line ExpandLine(Sci::Line line);
	void SetFoldExpanded(Sci::Line lineDoc, bool expanded);
	void FoldLine(Sci::Line line, Hyperion::FoldAction action);
	void FoldExpand(Sci::Line line, Hyperion::FoldAction action, Hyperion::FoldLevel level);
	Sci::Line ContractedFoldNext(Sci::Line lineStart) const noexcept;
	void EnsureLineVisible(Sci::Line lineDoc, bool enforcePolicy);
	void FoldChanged(Sci::Line line, Hyperion::FoldLevel levelNow, Hyperion::FoldLevel levelPrev);
	void NeedShown(Sci::Position pos, Sci::Position len);
	void FoldAll(Hyperion::FoldAction action);
	...
	virtual std::unique_ptr<Surface> CreateDrawingSurface(SurfaceID sid, std::optional<Hyperion::Technology> technologyOpt = {}) const;
	...
	virtual Hyperion::sptr_t DefWndProc(Hyperion::Message iMessage, Hyperion::uptr_t wParam, Hyperion::sptr_t lParam) = 0;
	bool ValidMargin(Hyperion::uptr_t wParam) const noexcept;
	void StyleSetMessage(Hyperion::Message iMessage, Hyperion::uptr_t wParam, Hyperion::sptr_t lParam);
	Hyperion::sptr_t StyleGetMessage(Hyperion::Message iMessage, Hyperion::uptr_t wParam, Hyperion::sptr_t lParam);
	void SetSelectionNMessage(Hyperion::Message iMessage, Hyperion::uptr_t wParam, Hyperion::sptr_t lParam);
	...
	static void *PtrFromSPtr(Hyperion::sptr_t lParam) noexcept {
		return reinterpret_cast<void *>(lParam);
	}
	static const char *ConstCharPtrFromSPtr(Hyperion::sptr_t lParam) noexcept {
		return static_cast<const char *>(PtrFromSPtr(lParam));
	}
	static const unsigned char *ConstUCharPtrFromSPtr(Hyperion::sptr_t lParam) noexcept {
		return static_cast<const unsigned char *>(PtrFromSPtr(lParam));
	}
	static char *CharPtrFromSPtr(Hyperion::sptr_t lParam) noexcept {
		return static_cast<char *>(PtrFromSPtr(lParam));
	}
	static unsigned char *UCharPtrFromSPtr(Hyperion::sptr_t lParam) noexcept {
		return static_cast<unsigned char *>(PtrFromSPtr(lParam));
	}
	static std::string_view ViewFromParams(Hyperion::sptr_t lParam, Hyperion::uptr_t wParam) noexcept {
	...
	static void *PtrFromUPtr(Hyperion::uptr_t wParam) noexcept {
		return reinterpret_cast<void *>(wParam);
	}
	static const char *ConstCharPtrFromUPtr(Hyperion::uptr_t wParam) noexcept {
		return static_cast<const char *>(PtrFromUPtr(wParam));
	}

	static constexpr Hyperion::sptr_t SPtrFromUPtr(Hyperion::uptr_t wParam) noexcept {
		return static_cast<Hyperion::sptr_t>(wParam);
	}
	static constexpr Sci::Position PositionFromUPtr(Hyperion::uptr_t wParam) noexcept {
		return SPtrFromUPtr(wParam);
	}
	static constexpr Sci::Line LineFromUPtr(Hyperion::uptr_t wParam) noexcept {
		return SPtrFromUPtr(wParam);
	}
	Point PointFromParameters(Hyperion::uptr_t wParam, Hyperion::sptr_t lParam) const noexcept {
		return Point(static_cast<XYPOSITION>(wParam) - vs.ExternalMarginWidth(), static_cast<XYPOSITION>(lParam));
	}

	static constexpr std::optional<FoldLevel> OptionalFoldLevel(Hyperion::sptr_t lParam) {
		if (lParam >= 0) {
	...
	static Hyperion::sptr_t StringResult(Hyperion::sptr_t lParam, const char *val) noexcept;
	static Hyperion::sptr_t BytesResult(Hyperion::sptr_t lParam, const unsigned char *val, size_t len) noexcept;
	static Hyperion::sptr_t BytesResult(Hyperion::sptr_t lParam, std::string_view sv) noexcept;
...
public:
	...
	virtual Hyperion::sptr_t WndProc(Hyperion::Message iMessage, Hyperion::uptr_t wParam, Hyperion::sptr_t lParam);
	// Public so Hyperion_set_id can use it.
	int ctrlID;
	// Public so COM methods for drag and drop can set it.
	Hyperion::Status errorStatus;
...
public:
	...
	AutoSurface(SurfaceID sid, const Editor *ed, std::optional<Hyperion::Technology> technology = {}) :
