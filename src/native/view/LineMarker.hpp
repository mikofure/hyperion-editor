...
typedef void (*DrawLineMarkerFn)(Surface *surface, const PRectangle &rcWhole, const Font *fontForCharacter, int tFold, Hyperion::MarginType marginStyle, const void *lineMarker);
...
public:
	...
	Hyperion::MarkerSymbol markType = Hyperion::MarkerSymbol::Circle;
	ColourRGBA fore = black;
	ColourRGBA back = white;
	ColourRGBA backSelected = ColourRGBA(maximumByte, 0, 0);	// Red default
	Hyperion::Layer layer = Hyperion::Layer::Base;
	Hyperion::Alpha alpha = Hyperion::Alpha::NoAlpha;
	XYPOSITION strokeWidth = 1.0f;
	std::unique_ptr<XPM> pxpm;
	std::unique_ptr<RGBAImage> image;
	/** Some platforms, notably PLAT_CURSES, do not support Hyperion's native
	...
	void Draw(Surface *surface, const PRectangle &rcWhole, const Font *fontForCharacter, FoldPart part, Hyperion::MarginType marginStyle) const;
