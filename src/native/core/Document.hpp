...
public:
	...
	virtual Sci::Position FindText(Document *doc, Sci::Position minPos, Sci::Position maxPos, const char *s,
                        bool caseSensitive, bool word, bool wordStart, Hyperion::FindOption flags, Sci::Position *length) = 0;
...
struct LexerReleaser {
	...
	void operator()(Hyperion::ILexer5 *pLexer) noexcept {
		if (pLexer) {
			try {
				pLexer->Release();
			} catch (...) {
...
using LexerInstance = std::unique_ptr<Hyperion::ILexer5, LexerReleaser>;

// LexInterface defines the interface to ILexer used in Document.
// The LexState subclass is actually created and that is used within HyperionBase
// to provide more methods that are exposed through Hyperion's external API.
class LexInterface {
protected:
	Document *pdoc;
...
public:
	...
	virtual Hyperion::LineEndType LineEndTypesSupported();
...
/**
 */
class Document : PerLine, public Hyperion::IDocument, public Hyperion::ILoader, public Hyperion::IDocumentEditable {
...
public:
	...
	Hyperion::EndOfLine eolMode;
	/// Can also be SC_CP_UTF8 to enable UTF-8 mode
	int dbcsCodePage;
	Hyperion::LineEndType lineEndBitSet;
	int tabInChars;
	int indentInChars;
	...
	Document(Hyperion::DocumentOption options);
	...
	Hyperion::LineEndType LineEndTypesSupported() const;
	bool SetDBCSCodePage(int dbcsCodePage_);
	Hyperion::LineEndType GetLineEndTypesAllowed() const noexcept { return cb.GetLineEndTypes(); }
	bool SetLineEndTypesAllowed(Hyperion::LineEndType lineEndBitSet_);
	Hyperion::LineEndType GetLineEndTypesActive() const noexcept { return cb.GetLineEndTypes(); }

	int SCI_METHOD Version() const override {
		return Hyperion::dvRelease4;
	}
	...
	static std::string TransformLineEnds(const char *s, size_t len, Hyperion::EndOfLine eolModeWanted);
	void ConvertLineEnds(Hyperion::EndOfLine eolModeSet);
	...
	Hyperion::DocumentOption Options() const noexcept;
	...
	Sci::Position IndexLineStart(Sci::Line line, Hyperion::LineCharacterIndexType lineCharacterIndex) const noexcept;
	Sci::Line LineFromPositionIndex(Sci::Position pos, Hyperion::LineCharacterIndexType lineCharacterIndex) const noexcept;
	...
	Hyperion::FoldLevel GetFoldLevel(Sci_Position line) const noexcept;
	void ClearLevels();
	Sci::Line GetLastChild(Sci::Line lineParent, std::optional<Hyperion::FoldLevel> level = {}, Sci::Line lastLine = -1);
	...
	Sci::Position FindText(Sci::Position minPos, Sci::Position maxPos, const char *search, Hyperion::FindOption flags, Sci::Position *length);
	const char *SubstituteByPosition(const char *text, Sci::Position *length);
	Hyperion::LineCharacterIndexType LineCharacterIndex() const noexcept;
	void AllocateLineCharacterIndex(Hyperion::LineCharacterIndexType lineCharacterIndex);
	void ReleaseLineCharacterIndex(Hyperion::LineCharacterIndexType lineCharacterIndex);
...
class DocModification {
public:
	Hyperion::ModificationFlags modificationType;
	Sci::Position position;
	Sci::Position length;
	Sci::Line linesAdded;	/**< Negative if lines deleted. */
	const char *text;	/**< Only valid for changes to text, not for changes to style. */
	Sci::Line line;
	Hyperion::FoldLevel foldLevelNow;
	Hyperion::FoldLevel foldLevelPrev;
	Sci::Line annotationLinesAdded;
	Sci::Position token;

	DocModification(Hyperion::ModificationFlags modificationType_, Sci::Position position_=0, Sci::Position length_=0,
		...
		text(text_),
		line(line_),
		foldLevelNow(Hyperion::FoldLevel::None),
		foldLevelPrev(Hyperion::FoldLevel::None),
		annotationLinesAdded(0),
		token(0) {}

	DocModification(Hyperion::ModificationFlags modificationType_, const Action &act, Sci::Line linesAdded_=0) noexcept :
		modificationType(modificationType_),
...
public:
	...
	virtual void NotifyErrorOccurred(Document *doc, void *userData, Hyperion::Status status) = 0;
